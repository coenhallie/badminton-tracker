...
The version is **derived from git at build time**. Nothing is hand-typed.
...
Bump `package.json` and create the tag **in the same commit**, so the fallback
version and the tag can never disagree. The build warns if they drift.
...
## The changelog is separate

The 12 changelog entries in `src/App.vue` are **not** derived from any of this.
Each is a historical label for a release that already shipped, written as prose for
end users. Deriving them would relabel old entries on every bump, and generated
commit logs are not the same artifact as a user-facing changelog.
